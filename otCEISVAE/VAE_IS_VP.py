...
def fitted_ae(X,y,latent_dim,epochs,batch_size,learning_rate,beta_1,beta_2,epsilon,dimLayer1,dimLayer2):
    input_dim = X.shape[1]
    ae = AutoEncoder(input_dim, latent_dim,dimLayer1,dimLayer2)
    ae.compile(optimizer=keras.optimizers.Adam(learning_rate=learning_rate,beta_1=beta_1,beta_2=beta_2,epsilon=epsilon))
...
def fitted_vae(X,y,latent_dim,K,epochs=100,batch_size=100,learning_rate=0.001,beta_1=0.9,beta_2=0.999, epsilon=1e-07,dimLayer1 = 32,dimLayer2 = 16):
    ...
    auto_encoder = fitted_ae(X_normed,y_normed,latent_dim,epochs,batch_size,learning_rate,beta_1,beta_2,epsilon,dimLayer1,dimLayer2)
    encoder,decoder = auto_encoder.get_encoder_decoder()
    
    vae = VAE(encoder, decoder, vp_layer, input_dim, latent_dim, K, mean_x,std_x)
    vae.compile(optimizer=keras.optimizers.Adam(learning_rate=learning_rate,beta_1=beta_1,beta_2=beta_2,epsilon=epsilon))
